...
function setActiveTree(key) {
    ...
    const delDom = formDom.getElementsByClassName('del-btn')[0];
    delDom && (delDom.style.display = 'inline-block');
}
/** 重置树列表状态 */
function resetActiveState(cb) {
...
/** 生成接口树 */
{
    ...
    function generateTreeContent(data) {
        source = data;
        const a = {};
        Object.entries(data)
    ...
    treeDom.addEventListener('click', (ev) => {
        target = ev.target;
        if (!(target.dataset.key && source[target.dataset.key]))
            return;
...
function setFormValue(key) {
    ...
    doms.forEach((el) => {
        ...
        el[el.type === 'checkbox' ? 'checked' : 'value'] =
            val[el.name] === undefined ? formDefaultValue[el.name] : val[el.name];
    });
...
{
    /** 创建输入框 */
    function createTextarea({ label, value, placeholder, tag, name, title, ...args }) {
    ...
    formDom.addEventListener('submit', function (ev) {
        ev.preventDefault();
        const params = [...new FormData(ev.target).entries()].reduce((p, v) => {
            // @ts-ignore
            p[v[0]] = v[1];
            return p;
        }, {});
...
const addBtn = getDom('add-btn');
const syncBtn = getDom('sync-btn');
const clearBtn = getDom('clear-btn');
const msgBox = getDom('msg-content');
// 绑定事件
addBtn.onclick = () => {
    resetActiveState();
    formDom.reset();
...
clearBtn &&
    (clearBtn.onclick = () => window.confirm('确认清空数据?') &&
        clearData(() => {
            formDom.reset();
            const delDom = formDom.getElementsByClassName('del-btn')[0];
            delDom && (delDom.style.display = 'none');
